...
不需要为整个程序决定选择 `Logger` 还是 `SugaredLogger` ，两者之间可以轻松转换。
...
问题 1: 没有打印。AtomicLevel 是原子性可更改的动态日志记录级别，通过调用 `atom.SetLevel` 更改日志级别为 `infoLevel` 。

问题 2: 没有打印。`zap.Development()` 启用了开发模式，在开发模式下 `DPanic` 函数会引发 panic，所以最后的 `error` 不会打印到控制台。
...
logger 提供快速，分级，结构化的记录。所有的方法都是安全的，内存分配很重要，因此它的 API 有意偏向于性能和类型。
...
`NewProductionEncoderConfig` 和 `NewDevelopmentEncoderConfig` 是返回编码器配置。
...
`Logger` 作为 `SugaredLogger` 的属性，这个封装优点在于性能充裕的情况下，快速调用日志。所以名字为加了糖的 Logger。
...
与Logger不同，SugaredLogger不坚持结构化日志。对于每个日志级别，提供了三种方法。
...
`Info` , `Infof`, `Infow` 三个函数都调用了 `log` 函数，代码如下
...
关于 `s.base.Check` ，这就要介绍到 zapcore ，下面分析相关模块。

#### zapcore

包 zapcore 定义并实现了构建 zap 的低级接口。通过提供这些接口的替代实现，外部包可以扩展 zap 的功能。
...
`Check` 函数有两个入参。第一个参数表示一条完整的日志消息，第二个参数为 nil 时会从 sync.Pool 创建的池中取*CheckedEntry 复用，避免分配内存。该函数内部调用 `AddCore` 实现获取 *CheckedEntry，最后调用 `Write` 写入日志消息。
...
##### 为什么要在Logger性能上花费这么多精力？

当然，大多数应用程序不会注意到Logger慢的影响：它们每次操作都需要几十或几百毫秒，因此额外的毫秒并不重要。

另一方面，*为什么不*使结构化伐木快速？与其他记录包相比`SugaredLogger`的使用并不困难，`Logger`使结构化记录在对性能敏感的环境中成为可能。在 Go 微服务的车队中，使每个应用程序的效率稍高一些，加起来会很快。

##### 为什么`Logger`和`SugaredLogger`接口？

不像熟悉的`io.Writer`和`http.Handler`、`Logger`和`SugaredLogger`接口将包括*许多*方法。正如[罗布·派克指出](https://www.translatoruser-int.com/translate?&to=zh-Hans&csId=b3fae7f2-d2b0-4ccc-adf5-fea7ad40b1af&usId=1168fc7e-f5c4-4357-8e0d-549e1ce877ab&dl=en&ref=_ct&ac=true&dt=2021%2f5%2f15 13%3a46&h=1b3Jv3HJ9KoDIEyyj49mSwN5149Eg9dB&a=https%3a%2f%2fgo-proverbs.github.io%2f)的，"界面越大，抽象越弱。接口也是僵化的-*任何*更改都需要发布新的主要版本，因为它会破坏所有第三方实现。

制作`Logger`和`SugaredLogger`混凝土类型不会牺牲太多的抽象性，它允许我们在不引入突破性更改的情况下添加方法。您的应用程序应该定义并依赖于一个界面，该界面仅包括您使用的方法。

##### 为什么我的一些日志丢失了？

启用取样时，通过 zap 故意丢弃日志。生产配置（如`NewProductionConfig()`使采样能够在一秒内导致重复日志取样。查看更多详细信息，了解为什么在["为什么使用示例应用日志"中启用采样](https%3a%2f%2fgithub.com%2fuber-go%2fzap%2fblob%2fmaster%2fFAQ.md%23why-sample-application-logs).

##### 为什么要使用示例应用程序日志？

应用程序经常会遇到错误，无论是因为错误还是因为用户使用错误。记录错误通常是一个好主意，但它很容易使这种糟糕的情况变得更糟：不仅您的应用程序应对大量错误，它还花费额外的CPU周期和I/O记录这些错误。由于写入通常是序列化的，因此在最需要时，记录会限制吞吐量。

采样通过删除重复的日志条目来解决这个问题。在正常情况下，您的应用程序会写出每个条目。但是，当类似的条目每秒记录数百或数千次时，zap 开始丢弃重复以保存吞吐量。

##### 为什么结构化的日志 API 除了字段之外还可以接收消息？

主观上，我们发现在结构化上下文中附带一个简短的描述是有帮助的。这在开发过程中并不关键，但它使调试和操作不熟悉的系统更加容易。

更具体地说，zap 的采样算法使用消息来识别重复的条目。根据我们的经验，这是随机抽样（经常在调试时删除您需要的确切条目）和哈希完整条目（代价高）之间的一个实际中间点。
